...
/// @dev Vote weight decays linearly over time. Lock time cannot be more than `MAXTIME`.
contract VotingEscrow is IERC721, IERC721Metadata, IVotes {
    ...
    function split(uint[] calldata amounts, uint _tokenId) external {
        ...
        uint value = uint(int256(_locked.amount));
        require(value > 0, "zero locked"); // dev: need non-zero value
        
        // reset supply. supply will be increased by _deposit_for later
        supply = supply - value;
        uint i;
        uint totalWeight = 0;
        ...
        require(unlock_time > block.timestamp, 'Can only lock until time in the future');
        require(unlock_time <= block.timestamp + MAXTIME, 'Lock time exceeds maximum');
        
        // mint to the current owner
        uint _value = 0;
        for(i = 0; i < amounts.length; i++){   
            ++tokenId;
            _tokenId = tokenId;
