...
contract PairTest is BaseTest {
    ...
    function votingEscrowMerge() public {
        ...
        assertEq(VSTOKEN.balanceOf(address(escrow)), 3 * TOKEN_1);
        console2.log(escrow.totalSupply());
        escrow.merge(3, 1);
        console2.log(escrow.totalSupply());
        assertGt(escrow.balanceOfNFT(1), 1990063075414519385);
    ...
    function deployPairFactoryGauge() public {
        ...
        VSTOKEN.approve(address(gaugeFactory), 15 * TOKEN_100K);
        ...
        staking = new TestStakingRewards(address(pair), address(VSTOKEN));
    ...
    function addGaugeAndBribeRewards() public {
        ...
        VSTOKEN.approve(address(gauge), PAIR_1);
        VSTOKEN.approve(address(xbribe), PAIR_1);
        VSTOKEN.approve(address(staking), PAIR_1);

        gauge.notifyRewardAmount(address(VSTOKEN), PAIR_1);
        xbribe.notifyRewardAmount(address(VSTOKEN), PAIR_1);
        staking.notifyRewardAmount(PAIR_1);

        assertEq(gauge.rewardRate(address(VSTOKEN)), 1653);
        // no reward rate, all or nothing
        // assertEq(xbribe.rewardRate(address(VSTOKEN)), 1653);
    ...
    function createLock2() public {
        ...
        assertGt(escrow.balanceOfNFT(1), 995063075414519385);
        assertEq(VSTOKEN.balanceOf(address(escrow)), 4 * TOKEN_1);
    }
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        VSTOKEN.approve(address(voter), PAIR_1);
    ...
    function minterMint() public {
        ...
        VSTOKEN.approve(address(staking), claimable);
    ...
    function gaugeClaimRewards() public {
        ...
        gauge.batchRewardPerToken(address(VSTOKEN), 200);
        vm.warp(block.timestamp + 1);
        uint256 earned = gauge.earned(address(VSTOKEN), address(owner));
        address[] memory rewards = new address[](1);
        rewards[0] = address(VSTOKEN);
        vm.warp(block.timestamp + 1);
    ...
    function minterMint2() public {
        ...
        token[0] = address(VSTOKEN);
        tokens[0] = token;
        voter.claimRewards(gauges, tokens);
        assertEq(gauge.rewardRate(address(VSTOKEN)), staking.rewardRate());
        console2.log(gauge.rewardPerTokenStored(address(VSTOKEN)));
    }
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        vm.warp(block.timestamp + 1);
        // uint256 earned = gauge.earned(address(VSTOKEN), address(owner3));
        address[] memory rewards = new address[](1);
        rewards[0] = address(VSTOKEN);
        owner3.getGaugeReward(address(gauge), address(owner3), rewards);
    ...
    function testGaugeClaimRewards3() public {
        ...
        pair.approve(address(gauge), PAIR_1);
        gauge.deposit(PAIR_1, 0);
        VSTOKEN.approve(address(gauge), VSTOKEN.balanceOf(address(owner)));
        gauge.notifyRewardAmount(address(VSTOKEN), VSTOKEN.balanceOf(address(owner)));
